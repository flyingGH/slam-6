...
class CurveFittingEdge
 ...
 public:
  ...
  void computeError() override {
    ...
    _error(0, 0) = _measurement - std::exp(abc_est[0] * _x * _x +
                                           abc_est[1] * _x + abc_est[2]);
  }
...
int main() {
  ...
  vector<double> x_data, y_true_data, y_obs_data;
  {
    ...
    for (uint16_t i = 0; i < N; i++) {
      ...
      y_obs_data.push_back(exp(ar * x * x + br * x + cr) +
...

  {
    ...
    for (const auto &x : x_data) {
      y_est_data.push_back(
          exp(abc_est[0] * x * x + abc_est[1] * x + abc_est[2]));
    }
