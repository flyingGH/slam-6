...
def main():
    ...
    cam = PinholeCamera(**config["inner_params"])
    vo = VisualOdometry(cam, f'{root_path}/poses/{seq_id}.txt')

    # image for trajectory drawing
    traj = np.zeros((600, 600, 3), dtype=np.uint8)

    for img_id in range(config["img_id_end"]):
        img = cv2.imread(f'{root_path}/sequences/{seq_id}/image_0/{str(img_id).zfill(6)}.png',
