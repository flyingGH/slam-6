...
from . import stereo, tracking
from .config import config


def read_calib_matrices(calib_path: str) -> np.ndarray:
    """
    カメラ内部パラメータ読み込み

    :param calib_path: ファイルパス
    :return: カメラ内部パラメータ
    """
    calib_df = pd.read_csv(calib_path, delim_whitespace=True, header=None)
    calib_matrices = [row[1:].values.reshape((3, 4))
...
def demo_stereo_matching():
    ...
    left_image_path = os.path.join(
        root_path, "sequences/00/image_0/000000.png")
    right_image_path = os.path.join(
        root_path, "sequences/00/image_1/000000.png")
    ...
    points = stereo_estimator.images2points(
        left_image, right_image, calib_matrices[1], draw=False)
